...
func (s *OpenAIGatewayService) handleErrorResponsePassthrough(
	...
	if shouldRedactOpenAIPassthroughErrorForClient(resp) {
...
func shouldRedactOpenAIPassthroughErrorForClient(resp *http.Response) bool {
...
func applyOpenAIStreamFailedErrorPassthroughRule(
	...
	rule := svc.MatchRule(platform, upstreamStatus, ruleBody)
	if rule == nil {
		return status, errType, errMsg, false
	}
	...
	if rule.PassthroughBody {
		if bodyMsg := strings.TrimSpace(gjson.GetBytes(ruleBody, "error.message").String()); bodyMsg != "" {
			errMsg = bodyMsg
		} else {
			errMsg = ErrorPassthroughClientMessage(rule, errMsg)
		}
	} else {
		errMsg = ErrorPassthroughClientMessage(rule, errMsg)
	}
