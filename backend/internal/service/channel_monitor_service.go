...
type ChannelMonitorService struct {
	...
	// quotaFetcher 由 wire 在服务构造后注入；nil 时配额检测按未配置处理。
...
func (s *ChannelMonitorService) RunCheck(ctx context.Context, id int64) ([]*CheckResult, error) {
	...
	checkMode := defaultCheckMode(m.CheckMode)
	if m.Provider == MonitorProviderOpenAI && checkMode == MonitorCheckModeProbe {
...
// runQuotaOnlyCheck performs one account quota lookup without sending an LLM request.
