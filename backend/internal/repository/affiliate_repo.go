...
func (r *affiliateRepository) ListAffiliateRebateRecords(ctx context.Context, filter service.AffiliateRecordFilter) ([]service.AffiliateAdminRebateRecord, int64, error) {
	client := clientFromContext(ctx, r.client)
...
LIMIT $`+fmt.Sprint(len(args)-1)+` OFFSET $`+fmt.Sprint(len(args)), args...)
	...
	items := make([]service.AffiliateAdminRebateRecord, 0)
	for rows.Next() {
		var item service.AffiliateAdminRebateRecord
		if err := rows.Scan(
			&item.OrderID,
			&item.OutTradeNo,
			&item.InviterID,
