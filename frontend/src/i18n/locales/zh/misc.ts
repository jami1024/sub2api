export default {
  ...
  payment: {
    ...
    balancePackages: {
      ...
      backToList: '返回余额包列表',
      supportRange: '支持范围',
      supportsCodexOnly: '仅支持 Codex 分组',
      supportsGeneralOnly: '仅支持 General 分组',
      noMixedScope: '不可与其他余额模式混用',
      currentScopeCodex: '当前余额模式：Codex',
      currentScopeGeneral: '当前余额模式：General',
      currentScopeHint: '仅可继续购买同余额模式的余额包。',
      unavailable: '当前不可购买',
      unavailableCurrentScope: '当前账户已有其他余额模式的余额，请先消耗完现有余额后再切换。',
      purchaseNotice: '购买后额度会充值到当前账户余额中，并受对应余额模式限制。',
      forceSwitch: '强制切换模式',
      ...
      modeGuideSubtitle: '购买前请先确认余额模式适用范围与切换规则。',
      modeGuideCodexTitle: '仅适用于 Codex 分组',
      modeGuideCodexDesc: '适合只面向 Codex 请求的余额，购买后仅可用于 Codex 分组。',
      modeGuideGeneralTitle: '仅适用于 General 分组',
      modeGuideGeneralDesc: 'General 余额适合同时使用 Claude 和 Codex 的场景，购买后按 General 分组规则使用。',
      noticeSingleMode: '同一时间仅支持一种余额模式。',
      noticeSwitch: '如果当前已有其他余额模式的余额，购买新模式余额包前需要先消耗完余额，或使用强制切换模式。',
      noticeForceSwitch: '强制切换模式后，旧模式余额会被清空，且不可恢复。',
    },
    tabSubscribe: '订阅',
    noPlans: '暂无可用订阅套餐',
