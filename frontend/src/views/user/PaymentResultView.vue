      ...
      <template v-else>
        ...
        <div v-if="order" class="rounded-xl bg-white p-5 shadow-sm dark:bg-dark-800">
          <div class="space-y-3 text-sm">
            ...
            <div v-if="hasAmountFields(order) && normalizedFeeRate > 0" class="flex justify-between">
...
const normalizedFeeRate = computed(() => {
  const raw = hasAmountFields(order.value) ? order.value.fee_rate : 0
...
const feeAmount = computed(() => {
  if (!hasAmountFields(order.value) || normalizedFeeRate.value <= 0) return 0
  const payAmount = typeof order.value.pay_amount === 'number' && Number.isFinite(order.value.pay_amount)
    ? order.value.pay_amount
    : 0
...
const creditedAmountPrefix = computed(() => {
  const orderType = hasAmountFields(order.value) ? order.value.order_type : ''
