          ...
          <div class="relative overflow-hidden border-b border-amber-100/80 bg-gradient-to-br from-amber-50/80 via-orange-50/50 to-yellow-50/30 px-8 py-6 dark:border-dark-700/50 dark:from-amber-900/20 dark:via-orange-900/10 dark:to-yellow-900/5">
            ...
            <div class="relative z-10">
              ...
              <div class="flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-400">
                ...
                <time>{{ formatTime(current.created_at) }}</time>
              </div>
            </div>
          </div>
...
import { listPublic } from '@/api/announcements'
import type { UserAnnouncement } from '@/types'

const STORAGE_KEY = 'public_announcements_dismissed'

const { t } = useI18n()

marked.setOptions({ breaks: true, gfm: true })

const current = ref<UserAnnouncement | null>(null)
const queue = ref<UserAnnouncement[]>([])
...
function handleDismiss() {
  if (current.value) {
    saveDismissedId(current.value.id)
  }
  current.value = null
...
onMounted(async () => {
  ...
  try {
    const announcements = await listPublic()
    const dismissed = getDismissedIds()
    const pending = announcements.filter(a => !dismissed.has(a.id))
    if (pending.length > 0) {
      current.value = pending[0]
      queue.value = pending.slice(1)
    }
  } catch {
    // silently ignore fetch errors for public announcements
  }
})
