  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <span v-if="settingsLoaded" class="sidebar-brand-title text-lg font-bold text-gray-900 dark:text-white">
          {{ siteName }}
        </span>
        <!-- Version Badge -->
        <VersionBadge v-if="settingsLoaded" :version="siteVersion" />
      </div>
    </div>
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/purchase', label: t('nav.purchaseSubscriptionMenu'), icon: RechargeSubscriptionIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    { path: '/orders', label: t('nav.myOrders'), icon: OrderListIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    { path: '/redeem', label: t('nav.redeem'), icon: GiftIcon, hideInSimpleMode: true },
    { path: '/affiliate', label: t('nav.affiliate'), icon: UsersIcon, hideInSimpleMode: true, featureFlag: flagAffiliate },
    { path: '/guide', label: t('nav.userGuide'), icon: BookIcon },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/provider-status', label: t('nav.providerStatus'), icon: SignalIcon, featureFlag: flagOpsMonitoring },
    { path: '/admin/client-failures', label: t('nav.clientFailures'), icon: ShieldIcon, featureFlag: flagOpsMonitoring },
...
function isGroupActive(item: NavItem): boolean {
  ...
  return isGroupActiveByPath(route.path, item.children.map(child => child.path))
}
